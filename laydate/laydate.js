...
var config =  {
    ...
    min: '2017-8-19 23:59:59', //最小日期
    max: '2017-9-30 23:59:59', //最大日期
    isv: false,
    init: true,
	start: '2017-8-19 23:59:59',
};
...
Dates.events = function(){
    ...
    Dates.each(S(log.box + ' .laydate_ym'), function(i, elem){
        Dates.on(elem, 'click', function(ev){
            Dates.stopmp(ev).reshow();
            Dates.addClass(this[tags]('div')[0], '');
            if(!i){
    ...
    Dates.each(S('#laydate_table td'), function(i, elem){
		...
		document.getElementById("choosedatefinal").innerHTML = document.getElementById("laydate-icon").value;
		if(document.getElementById("laydate-icon").value != "" && document.getElementById("choosetime").value != ""){
		document.getElementById("noteice0301").innerHTML = "，实际上门安装时间请以装维人员沟通为准。";
				}
